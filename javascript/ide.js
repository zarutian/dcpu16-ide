...
function init() {
	...
	emulator.verbose = false;
	emulator.paused = true;
	var m1 = new Monitor(emulator);
	// var m2 = new Monitor(emulator);
	mediaDrive = new HMD2043(emulator);
	document.getElementById("monitor1").appendChild(m1.getDOMElement());
	// document.getElementById("monitor2").appendChild(m2.getDOMElement());
	document.getElementById("media-drive").appendChild(mediaDrive.getDOMElement());
	
	emulator.devices.push(m1);
	emulator.devices.push(new Keyboard(emulator));
	emulator.devices.push(new Clock(emulator));
	emulator.devices.push(mediaDrive);
	var exmem1 = new SVMM(emulator);
	emulator.devices.push(exmem1);
	var exmem2 = new SVMM(emulator);
	emulator.devices.push(exmem2);
	// emulator.devices.push(m2);
